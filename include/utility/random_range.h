...
struct FloatRange {
    float min;
    float max;
    FloatRange() : min(0.0f), max(0.0f) {};
    FloatRange(float min, float max) : min(min), max(max) {
        assert(min <= max || min != min); // ranges with NaN are valid and always output NaN
    }
...
struct Vec2Range {
    Vector2 min;
    Vector2 max;
    Vec2Range() : min(VEC2_ZERO), max(VEC2_ZERO) {};
...
struct ColorRange {
    Color from;
    Color to;
    ColorRange() : from(WHITE), to(WHITE) {};
